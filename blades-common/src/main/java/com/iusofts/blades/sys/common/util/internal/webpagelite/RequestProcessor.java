...
package com.iusofts.blades.sys.common.util.internal.webpagelite;
...
import com.iusofts.blades.sys.common.util.io.StreamUtil;
import com.iusofts.blades.sys.common.util.templatelite.Template;
import com.iusofts.blades.sys.common.util.templatelite.TextWriter;
...
public abstract class RequestProcessor<RC extends RequestContext> implements PageComponentRegistry {
    ...
    static {
        contentTypes = CollectionUtil.createHashMap();
    ...
    public void register(String componentPath, PageComponent component) {
        Assert.assertNotNull(componentPath, "componentPath is null");
        Assert.assertTrue(componentPath.length() == 0 || !componentPath.startsWith("/") && componentPath.endsWith("/"),
                   "invalid componentPath: %s", componentPath);
        Assert.assertTrue(!components.containsKey(componentPath), "duplicated component: %s", componentPath);
    ...
    public <PC extends PageComponent> PC getComponent(String componentPath, Class<PC> componentClass) {
        ...
        PageComponent component = Assert.assertNotNull(components.get(componentPath), "Component not found: %s", componentPath);
    ...
    private URL getRawResource(String resourceName) {
        ...
        URL resource = null;
        Set<String> visitedPackages = CollectionUtil.createHashSet();
    ...
    private void renderResource(final RC request, URL resource, boolean template) throws IOException {
        ...
        if (!template) {
            StreamUtil.io(resource.openStream(), request.getOutputStream(contentType), true, true);
        } else {
    ...
    protected List<String> getComponentResources(String ext) {
        List<String> names = CollectionUtil.createLinkedList();

        ext = StringUtil.trimToEmpty(ext);
