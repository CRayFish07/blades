...
import static com.iusofts.blades.sys.common.util.ArrayUtil.*;
import static com.iusofts.blades.sys.common.util.BasicConstant.*;
import static com.iusofts.blades.sys.common.util.StringEscapeUtil.*;
...
import com.iusofts.blades.sys.common.util.FileUtil;
import com.iusofts.blades.sys.common.util.ToStringBuilder;
import com.iusofts.blades.sys.common.util.Assert;
import com.iusofts.blades.sys.common.util.CollectionUtil;
...
public final class Template {
    ...
    private final static Map<String, Template> predefinedTemplates = CollectionUtil.createHashMap();
    ...
    private Template(InputSource source, String name) {
        ...
        this.source = Assert.assertNotNull(source, "source");
    ...
    private Template(String name, Node[] nodes, Map<String, String> params, Map<String, Template> subtemplates,
        ...
        this.name = trimToNull(name);
        this.source = null;
        this.location = Assert.assertNotNull(location, "location");
        this.ref = null;
    ...
    private Template(Template ref) {
        this.ref = Assert.assertNotNull(ref, "template ref");
        this.name = null;
        this.source = null;
        this.location = null;
        this.nodes = null;
    ...
    private void assertNotRef() {
        Assert.assertNull(ref, Assert.ExceptionType.UNSUPPORTED_OPERATION, "template ref");
    }
    ...
    private void update(Node[] nodes, Map<String, String> params, Map<String, Template> subtemplates) {
        ...
        this.params = CollectionUtil.createArrayHashMap(Assert.assertNotNull(params, "params").size());
        this.subtemplates = CollectionUtil.createArrayHashMap(Assert.assertNotNull(subtemplates, "subtemplates").size());
    ...
    private void invokeVisitor(Object visitor, Object node, int redirectDepth) throws TemplateRuntimeException {
        ...
        Class<?> visitorClass = Assert.assertNotNull(visitor, "visitor is null").getClass();
        ...
        try {
            ...
            // 3. visitXyz(Template, String, String, Template, ...) // 独立参数
                ...
                Assert.unreachableCode("Unexpected node type: " + node.getClass().getName());
            }
    ...
    private Object[] toPlaceholderParameterValues(Placeholder placeholder, Object[] params) {
        for (int i = 0; i < params.length && i < placeholder.params.length; i++) {
            ...
            if (param.isTemplateReference()) {
                params[i] = Assert.assertNotNull(param.getTemplateReference());
            } else {
    ...
    static class Text extends Node {
        ...
        public Text(String text, Location location) {
            super(location);
            this.text = Assert.assertNotNull(text, "text is null");
        }
    ...
    static class Placeholder extends Node {
        ...
        public Placeholder(String name, String paramsString, Location location) {
            super(location);
            this.name = Assert.assertNotNull(trimToNull(name), "${missing name}");
    ...
    static class IncludeTemplate extends Node {
        ...
        public IncludeTemplate(String templateName, Location location) {
            super(location);
            this.templateName = Assert.assertNotNull(trimToNull(templateName), "$#{missing template name}");
        }
    ...
    private static class Parser {
        ...
        private final static Set<String> KEYWORDS = CollectionUtil.createTreeSet("text", "placeholder", "template", "end");
        ...
        public Parser(Reader reader, String systemId, InputSource source) {
            this.source = Assert.assertNotNull(source, "input source");
            this.systemId = trimToNull(systemId);
        ...
        private void postProcessNode(Node node, LinkedList<Map<String, Template>> templateStack) {
            ...
            if (node instanceof Placeholder && !isEmptyArray(((Placeholder) node).params)) {
                List<PlaceholderParameter> expandedParameters = CollectionUtil.createLinkedList();
