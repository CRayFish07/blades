...
package com.iusofts.blades.sys.common.util.i18n;
...
public final class LocaleInfo implements Cloneable, Externalizable {
    ...
    public static LocaleInfo parse(String name) {
        name = Assert.assertNotNull(StringUtil.trimToNull(name), "no locale name");
    ...
    /** 创建系统默认的locale信息。 */
    public LocaleInfo() {
        this.locale = Assert.assertNotNull(Locale.getDefault(), "system locale");
        this.charset = Assert.assertNotNull(Charset.defaultCharset(), "system charset");
    }
    ...
    static class UnknownCharset extends Charset {
        public UnknownCharset(String name) {
            super(Assert.assertNotNull(name, "charset name"), null);
        }
        ...
        public CharsetDecoder newDecoder() {
            Assert.unsupportedOperation("Could not create decoder for unknown charset: " + name());
            return null;
        }

        @Override
        public CharsetEncoder newEncoder() {
            Assert.unsupportedOperation("Could not create encoder for unknown charset: " + name());
            return null;
        }
    }
}
