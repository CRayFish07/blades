...
import com.iusofts.blades.sys.common.util.Encodes;
...
import com.iusofts.blades.sys.web.util.config.Global;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (user != null) {
			...
			if (Global.NO.equals(user.getEnabled())){
				throw new AuthenticationException("msg:该已帐号禁止登录.");
			}
			
			//密码的加密方式与用户新增时加密方法一致，请勿擅自修改
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (user != null) {
			...
			//info.addStringPermission("/sys/user");
			//info.addStringPermission("/sys/role");
			info.addStringPermission("/sys/main");
			info.addStringPermission("/sys/top");
			info.addStringPermission("/sys/left");
...
//		}
	...
	public SessionDAO getSessionDao() {
		if (sessionDao == null){
			sessionDao = (SessionDAO) ServiceLocator.init().getService(SessionDAO.class);
		}
		return sessionDao;
	}
