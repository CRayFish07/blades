...
<body>
    ...
    <div class="formtitle"><span>添加<c:if test="${porg!=null }">『${porg.name }』下级</c:if>机构</span></div>
    <form id="myForm" method="post" action="save.do">
    <ul class="forminfo">
    <li><label>机构名称<b>*</b></label><input name="name" type="text" class="dfinput" /><i id="msg_name">名称不能超过30个字符</i></li>
    <li><label>机构别称</label><input name="alias" type="text" class="dfinput" /><i id="msg_alias"></i></li>
    <li><label>机构代码<b>*</b></label><input name="code" type="text" class="dfinput" /><i id="msg_code">代码是唯一的</i></li>
